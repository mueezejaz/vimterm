...
Each tab runs its own shell; output keeps accumulating while a tab is in the background. `gt` switches to the next tab, `gT` to the previous one, and `{count}gt` jumps to tab N. With more than one tab open, the status line lists them (the active one highlighted); with one tab the bar looks as before. Closing the last tab exits vimterm.
